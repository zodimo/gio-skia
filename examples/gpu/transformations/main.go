...
func Run(window *app.Window) error {
	...
	for {
		...
		case app.FrameEvent:
			...
			for i := 0; i < 8; i++ {
				c.Save()
				c.Rotate(float32(i) * 180.0 / 4) // Convert radians to degrees
				c.Translate(80, 0)
			...
			for i := 0; i < 12; i++ {
				c.Save()
				c.Rotate(float32(i) * 180.0 / 6) // Convert radians to degrees
				c.Translate(50, 0)
				skPaint := skia.NewPaintFill(color.NRGBA{
					R: uint8(150 + i*8),
					G: uint8(200 - i*10),
			...
			for i := 0; i < 6; i++ {
				c.Save()
				c.Rotate(float32(i) * 180.0 / 3) // Convert radians to degrees
				c.Translate(40, 0)
				c.Rotate(float32(i) * 180.0 / 6) // Convert radians to degrees
				skPaint := skia.NewPaintFill(color.NRGBA{
					R: uint8(200 + i*8),
			...
			for i := 0; i < 5; i++ {
				c.Save()
				c.Rotate(float32(i) * 180.0 / 8) // Convert radians to degrees
